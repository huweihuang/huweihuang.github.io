...
`kubelet`的工作是管理`Pod`在Node上的生命周期（包括增删改查），`kubelet`通过各种类型的`manager`异步工作各自执行各自的任务，其中使用到了多种的`channel`来控制状态信号变化的传递，例如比较重要的channel有`podUpdates <-chan UpdatePodOptions`，来传递pod的变化情况。
